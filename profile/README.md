# PublicSchema

**A schema.org-style vocabulary of concepts, properties, and controlled value sets for government service delivery.**

PublicSchema gives governments, development organizations, and technology partners a shared language for describing people, programs, enrollments, payments, and the other building blocks of service delivery. Starting with social protection, extending to broader government services over time.

## What's in the vocabulary

- **Concepts** like Person, Household, Enrollment, and PaymentEvent, each with a plain-language definition, properties, and data classification guidance.
- **Properties** that are independent and reusable across concepts, with clear semantics and sensitivity labels.
- **Vocabularies** (controlled value sets) that reference international standards (ISO, FHIR, UN) where they exist, and define new sets only where no standard covers the domain.

## Design principles

- **Descriptive, not prescriptive.** Everything is optional. Systems adopt what they need.
- **Plain language.** Definitions are written for policy officers, not developers.
- **Standards-aligned.** Built on JSON-LD with stable URIs, designed for use as Verifiable Credential schemas.
- **Evidence-based.** Informed by convergence analysis across real-world social protection systems.

## Get involved

- Browse the vocabulary at [publicschema.org](https://publicschema.org)
- Read the [contribution guide](https://github.com/PublicSchema/publicschema.org/blob/main/CONTRIBUTING.md)
- Open an [issue](https://github.com/PublicSchema/publicschema.org/issues) or join the [discussion](https://github.com/PublicSchema/publicschema.org/discussions)

## License

Vocabulary definitions: [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) | Code and tooling: [Apache 2.0](https://www.apache.org/licenses/LICENSE-2.0)
